...
def stSpectralFlatness(X):
    """
    Computes the spectral flatness of a frame, given the fft mag
    ...
    mspecSquare = numpy.square(X) + eps
    sfm = 20 * numpy.log10(gmean(mspecSquare) / numpy.mean(mspecSquare))
    return sfm
...
def stFeatureExtraction(signal, Fs, Win, Step):
    ...
    totalNumOfFeatures = numOfTimeSpectralFeatures + nceps + numOfHarmonicFeatures + numOfChromaFeatures + 1
#    totalNumOfFeatures = numOfTimeSpectralFeatures + nceps + numOfHarmonicFeatures
    ...
    while (curPos + Win - 1 < N):                        # for each short-term window until the end of signal
        ...
        curFV[numOfTimeSpectralFeatures + nceps + numOfChromaFeatures] = stSpectralFlatness(X);
